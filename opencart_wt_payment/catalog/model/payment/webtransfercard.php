<?php 
class ModelPaymentWebtransfercard extends Model {
  	public function getMethod($address) {
		$this->load->language('payment/webtransfercard');
		
		if ($this->config->get('webtransfercard_status')) {
			...
			if (!$this->config->get('webtransfercard_geo_zone_id')) {
        		$status = TRUE;
		...
		if ($status) {  
      		$method_data = array( 
        		'code'         => 'webtransfercard',
        		'title'      => $this->language->get('text_title'),
				'sort_order' => $this->config->get('webtransfercard_sort_order')
      		);
    	}
